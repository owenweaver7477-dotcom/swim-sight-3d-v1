...
| 7 | **Phase 3.1b — "Powered by" as a per-tier setting (audit S5).** Added `showsReportAttribution(planKey)` derived from the **existing `clubs.plan_key`** — **no migration needed**. Free/pilot tiers keep the attribution; paid club tiers (Club Pro, AI Assist, Elite) get a report headed by the **club's own logo + name** instead. Applied to `PrintableReport` (the PDF that actually gets handed to parents — the audit's specific concern). **Escalated, not done:** the shared-link page's attribution, because `api/shared-reports/[token].js` deliberately exposes only `club: { name }` and gating it would require changing that **protected** endpoint's payload (a computed boolean, never `plan_key` — leaking a commercial tier to an anonymous viewer would be wrong). Needs owner approval. | featureGates, PrintableReport | `lint && build` OK · **18/18 guards** incl. public-report-safety | `ece8477` |
| 8 | **Task A — finish the drill library (non-destructive).** ⚠️ **Correction to entry 6:** I previously stated the 9 `shared_default` DB rows were "added to, not merged over" the bundled defaults. **That was wrong.** All 9 ids (`bs-heel-recovery-pause`, `fr-front-scull`, …) **collide exactly** with bundled default ids, and `mergeDrills` let the DB row win — so 9 rich bundled drills (purpose, coaching cue, execution steps) were being **replaced** by placeholder rows carrying only a uniform "4 x 25m easy technique". That is the true root cause of the audit's screenshot. Fixed **in code, no DB write**: a `shared_default` row may no longer overwrite a bundled default of the same id; a club's own drill (`club_id` set) still always wins. Also added a graceful badge fallback chain **difficulty → phase → category** so the ~33 drills without a difficulty never render a blank/undefined chip. | DrillLibrary | `lint && build` OK · **18/18 guards** · simulated the shipped merge over the real 107 bundled + real 9 DB rows: **0 junk rows survive, 0 blank badges, 23 distinct badge labels** (badge sources: 74 difficulty / 18 phase / 15 category) | `b4ff9da` |
| 9 | **Task B — shared-report attribution via computed boolean (owner-approved).** The endpoint now fetches `plan_key` **server-side only**, derives the decision with `showsReportAttribution()`, and emits **only** `club.show_attribution`. Imported the helper from `src/lib/plans/featureGates.js` rather than duplicating the tier list — the endpoint already cross-imports `src/lib/sanitizeAIReport.js`, so this is the established pattern and leaves **one source of truth, no drift risk**. Discovered the sanitizer builds `club` from an **allowlist**, which means `plan_key` was *already* unable to leak (defence in depth) — but also meant the new boolean needed adding to that allowlist. Checked `compactObject` first: it strips `null`/`undefined`/`''` but **not `false`**, so an explicit boolean survives (a `false` would otherwise have been dropped and silently re-enabled attribution). | shared-reports/[token], featureGates, sanitizeAIReport, SharedReportPage | `lint && build` OK · **18/18 guards** | `ca6f53e` |
| 10 | **Task B (guard) — assert the commercial tier can never reach a public payload.** Extended `check_public_report_safety.mjs`: feeds a payload carrying `plan_key`/`plan`/`tier`/`subscription`/`billing_plan`/`plan_name` and asserts none survive, that no tier *value* appears anywhere in the JSON, that the boolean survives (incl. `false`), that it defaults to shown when absent, and that the endpoint still derives it server-side and never emits `plan_key`. **Proved the guard bites:** injecting `plan_key` into the sanitizer allowlist fails it with *"Public payload must not expose the club's commercial tier"*; file restored and re-verified clean. | check_public_report_safety | guard passes · **failure case verified** | `ca6f53e` |
| 11 | **Task C — Example ("demo squad") mode, FRONT-END ONLY (owner-approved; audit S2 + S6).** New `demoData.js` (10 swimmers across Gold/Silver with mixed strokes and staggered last-reviewed dates, 2 squads, 2 **finalised** reports and 3 findings using the full content model — What we saw / Why it matters / What to feel / Recommended drill / Next focus) and `demoMode.js` (localStorage state, subscribe/notify, `DemoModeWriteError`). Intercepted at **`createEntityAdapter`** — the single choke point every read and write funnels through — so *all* screens populate from one change: `list`/`filter`/`get` serve in-memory rows **before** Supabase is called, and `create`/`update`/`delete` **throw**. Club context returns the demo club while active. Always-visible amber banner + one-click exit; entry point on the zero-data setup screen. Drills already come from the bundled defaults, so the library is populated too. | demoData (new), demoMode (new), DemoModeBanner (new), entities, useClubContext, AppLayout, TeamDashboard | `lint && build` OK · **18/18 guards** · **proved read-only**: all 3 write paths refuse, all 3 read paths short-circuit before Supabase, all records scoped to `DEMO_CLUB_ID` with no real club id present | `fa81813` |

---

# Branch: `feat/finding-composer`

Branched off `main` at `5d434d0` (launch-hardening batch + DE-1 auth fix).

| # | What | Files | Verification | Commit |
|---|---|---|---|---|
| 1 | **Demo-mode read-only, proven by exercise — and a real hole found.** ⚠️ **Correction to entry 11 above.** That entry claimed demo mode was "proved read-only: all 3 write paths refuse". The proof was static analysis, and it enumerated **3** write methods when the adapter has **5** — `bulkCreate` and `softDelete` were never checked. `bulkCreate` had **no `refuseDemoWrite` call at all**, so in demo mode it reached Supabase like any real write. That gap shipped to production in the launch-hardening batch. Fixed, and the class of mistake fixed with it: new guard `check_demo_mode_readonly.mjs` **imports the real data layer and calls every mutating method**, rather than reading the source. The check is **two-sided** — with demo mode on each call must fail with `DemoModeWriteError`; with it off the *same* call must fail with the Supabase config error, which is what proves the call genuinely reaches Supabase when it is not refused. A one-sided check cannot distinguish "refused" from "never ran". New `scripts/_src_loader.mjs` (Node module hooks mapping Vite's `@/` alias and `import.meta.env`) is what lets a guard run real `src/` code; no new dependency. | entities, check_demo_mode_readonly (new), _src_loader + hooks (new), package.json, CLAUDE.md | `lint && build` OK · **19/19 guards** · **guard proved to bite**: run before the fix, it failed on `bulkCreate` for all 6 entities with *"That message means it REACHED Supabase"*; after the fix, 30/30 writes refused and all 5 methods confirmed to reach Supabase with demo mode off | `ee7752b` |
