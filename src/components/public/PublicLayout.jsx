...
// Shared eyebrow — the one place the accent colour appears in a section header.
// Small, uppercase, letter-spaced: a Swiss-style label, used once per section.
const eyebrowClass = 'reveal text-xs font-semibold uppercase tracking-[0.18em] text-sky-700';

export function PublicHero({ eyebrow, title, description, actions, children }) {
  return (
    <section className="border-b border-slate-200 bg-white">
      <div className={`mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:py-20 md:py-24 ${children ? 'lg:grid-cols-[1.1fr_0.9fr] lg:items-center' : ''}`}>
        <div>
          {eyebrow && <div className={eyebrowClass}>{eyebrow}</div>}
          <h1 className="reveal mt-4 max-w-3xl text-4xl font-bold leading-[1.05] tracking-tight text-slate-950 sm:text-5xl md:text-6xl">{title}</h1>
          <p className="reveal mt-6 max-w-2xl text-base leading-8 text-slate-600 md:text-lg">{description}</p>
