  ...
  return (
    <PublicLayout>
      ...
      <PublicHero
        ...
        actions={(
          <>
            <a href={PILOT_MAILTO} className="rounded-full bg-slate-950 px-5 py-3 text-center text-sm font-semibold text-white hover:bg-slate-800">Book a pilot</a>
            <Link to="/sample-report" className="rounded-full border border-slate-300 px-5 py-3 text-center text-sm font-semibold text-slate-700 hover:bg-slate-100">View sample report</Link>
          </>
        )}
      />
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {tiers.map(({ plan, items, note, featured }) => (
            <article key={plan.key} className={`rounded-3xl border p-5 shadow-sm ${featured ? 'border-sky-200 bg-sky-50' : 'border-slate-200 bg-white'}`}>
              ...
              <a href={PILOT_MAILTO} className="mt-4 inline-flex w-full justify-center rounded-full bg-slate-950 px-4 py-2.5 text-xs font-semibold text-white hover:bg-slate-800">
                {plan.ctaLabel}
              </a>
            </article>
          ))}
