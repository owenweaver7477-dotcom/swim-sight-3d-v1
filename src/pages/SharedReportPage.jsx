...
function PublicAnnotationCard({ annotation, linkedFindingTitle }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white overflow-hidden print:break-inside-avoid">
      {safeThumbnail ? (
        <div className="relative mx-auto w-full overflow-hidden bg-slate-100" style={boxStyle}>
        ...
        <div
          className="mx-auto w-full overflow-hidden bg-slate-100 [&>svg]:block [&>svg]:h-full [&>svg]:w-full"
          style={boxStyle}
      ...
      <div className="p-3">
        <div className="text-[10px] font-bold uppercase tracking-wider text-sky-700 mb-1 flex items-center gap-1.5">
        ...
        <div className="mt-1 flex flex-wrap gap-1.5">
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-mono font-semibold text-slate-600">
...
function PublicReportContent({ report, swimmer, club, video_meta, findings, annotations = [], dragItems = [] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-sm border border-slate-200 overflow-hidden sm:rounded-lg">
      {/* Header */}
      <div className="px-4 py-6 bg-white text-slate-950 border-b border-slate-200 print:border-b-2 print:border-slate-900 sm:px-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-5">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                <div className="text-xs font-semibold tracking-wider uppercase text-sky-700">Swim Sight 3D</div>
                <div className="text-[10px] text-slate-500">Coach-reviewed swim analysis</div>
              </div>
            </div>
            <h1 className="text-2xl font-bold tracking-tight mb-2 sm:text-3xl">{report.title || 'Swimmer Improvement Plan'}</h1>
            {club?.report_intro && (
              <p className="max-w-2xl text-sm italic leading-6 text-slate-600">{club.report_intro}</p>
            )}
            {report.coach_summary && (
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{report.coach_summary}</p>
            )}
          </div>
          <div className="flex w-fit items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider">Coach Approved</span>
          </div>
        </div>
        <div className="grid gap-4 pt-4 border-t border-slate-200 sm:grid-cols-3">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">Club</div>
            <div className="flex items-center gap-2">
              {club?.logo_url && <img src={club.logo_url} alt="" className="h-6 w-6 rounded object-contain bg-slate-100 p-0.5" />}
              <span className="font-semibold">{club?.name || 'Swim Club'}</span>
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">Swimmer</div>
            <div className="font-semibold">{swimmer?.name || '—'}</div>
          </div>
          <div className="sm:text-right">
            <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-0.5">Report Date</div>
      ...
      {/* Athlete info */}
      <div className="px-4 py-5 bg-slate-50 border-b border-slate-200 sm:px-8">
        ...
        {weeklyPlan.focus || weeklyPlan.cues.length || weeklyPlan.drills.length ? (
          <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
              <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-sky-700">Coach focus</div>
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.22em] text-sky-700">
              ...
              {weeklyPlan.drills.length ? (
                <div className="mt-3 grid gap-2">
                  {weeklyPlan.drills.map(drill => (
                    <div key={drill} className="rounded-lg border border-slate-200 bg-white p-3 text-sm font-semibold leading-6 text-slate-800">
                      {drill}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="mt-3 text-sm leading-6 text-slate-600">Drill recommendations will appear here once selected by the coach.</p>
              )}
              {weeklyPlan.avoid && (
      ...
      {aiUsed && report.overall_score != null && report.overall_score > 0 && (
        <div className="px-4 py-6 border-b border-slate-200 sm:px-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            <div className="w-24 h-24 rounded-lg bg-slate-950 flex flex-col items-center justify-center text-white">
      ...
      {keyMoments.length > 0 && (
        <div className="px-4 py-6 border-b border-slate-200 sm:px-8">
          <h2 className="text-lg font-bold text-slate-900 uppercase tracking-wider mb-5 flex items-center gap-2">
            <span className="w-1 h-6 bg-sky-700 rounded-full"></span>
        ...
        <div className="px-4 py-6 sm:px-8">
          ...
          <div className="space-y-4">
            {findings.map((finding, index) => {
              ...
              return (
              <div key={index} className="rounded-lg border border-slate-200 bg-white overflow-hidden print:break-inside-avoid print:border-slate-300">
                ...
                <div className="px-5 py-4 space-y-3.5">
                  ...
                  {finding.cue && (
                    <div className="pl-4 border-l-4 border-sky-700 bg-slate-50 rounded-r-lg p-3">
                      <div className="text-[10px] font-bold uppercase tracking-wider text-sky-700 mb-1.5 flex items-center gap-1.5">
                  ...
                  {finding.drill && (
                    <div className="flex flex-col gap-2.5 bg-slate-50 rounded-lg px-4 py-2.5 border border-slate-200 sm:flex-row">
                      <Dumbbell className="w-4 h-4 text-sky-700 flex-shrink-0 mt-0.5" />
                      <div>
                        <div className="text-[10px] font-bold uppercase tracking-wider text-sky-700 mb-0.5">Recommended Drill</div>
                        ...
                        {finding.linked_drill_summary && (
                          <div className="mt-2 rounded-lg bg-white border border-slate-200 px-3 py-2">
                            <div className="text-[10px] font-bold uppercase tracking-wider text-sky-700">Why it helps</div>
                        ...
                        {Array.isArray(finding.extra_drills) && finding.extra_drills.length > 0 && (
                          <div className="mt-2 rounded-lg bg-white border border-slate-200 px-3 py-2">
                            <div className="text-[10px] font-bold uppercase tracking-wider text-sky-700">Additional drills</div>
      ...
      {/* Footer */}
      <div className="px-4 py-5 bg-slate-50 border-t border-slate-200 sm:px-8">
...
export default function SharedReportPage() {
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="max-w-sm w-full text-center space-y-4">
          <div className="w-16 h-16 rounded-lg bg-destructive/10 flex items-center justify-center mx-auto">
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Top branding bar */}
      <div className="bg-white border-b border-slate-200 px-4 py-4 print:hidden sticky top-0 z-50">
        <div className="max-w-4xl mx-auto flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            {club?.logo_url ? (
              <img src={club.logo_url} alt={club.name} className="w-9 h-9 rounded-lg object-contain bg-slate-100 p-1" />
            ) : (
              <div className="w-9 h-9 rounded-lg bg-slate-950 flex items-center justify-center">
                <Waves className="w-5 h-5 text-white" />
              </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-200">
              <CheckCircle2 className="w-4 h-4 text-emerald-600" />
              <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">Coach Approved</span>
            </div>
            <Button
              size="sm"
              variant="outline"
              className="text-xs border-slate-300 text-slate-700 hover:bg-slate-50 print:hidden"
