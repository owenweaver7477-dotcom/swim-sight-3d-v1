...
- [ ] **P0-3 · Translate internal language into coach language** — 🟠 S3 · High · Low — *mostly done*
  ✅ "Quality gate failed — manual review recommended" → **"Best reviewed by you"**.
  ✅ **"V1 COACH FLOW" — already gone** (removed in the 1.1b copy purge; 0 hits repo-wide).
  ✅ **"AI credit" hidden until billing exists.** Under the pilot lock the indicator could only ever read *"Manual review: 0 AI credits"* — a billing concept the coach cannot act on, shown on every report forever. Now gated behind `!AI_PILOT_LOCKED` on `AIReportPage`, so it returns automatically when AI unlocks. The component and its `Analyse` usage were deliberately left alone: both are pinned by `check_pilot_recovery_foundation` and `check_analyse_route_runtime_safety`, and the Analyse instance already sits inside the `!AI_PILOT_LOCKED` branch, so it is dormant anyway.
  ⚠️ **"backend-assisted" — checked, and it should NOT be removed.** The only rendered instance is `PilotReadinessWarning`, which renders solely on `PilotLaunchPage` (internal, not in the coach nav) — exactly where P0-1 deliberately moved it. The other hit is an `internalNote` string in `featureReadiness.js`, never rendered. Removing either would delete real system status from the admin page the audit asked to keep it on.
  ⬜ Remaining: status vocabulary ("queued for AI, processing, needs retry") — deferred until AI unlocks, since none of those states are reachable under the manual-first lock.
