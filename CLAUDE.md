...
- **Shared UI components:** `src/components/ui/` — the surviving shadcn-style primitives are `badge`, `button`, `checkbox`, `dialog`, `input`, `label`, `select`, `sheet`, `sonner`, `textarea`, `tooltip`. Reuse these before hand-rolling; 34 unused primitives were deleted in the dead-code sweep (`ead8384`), so if a shadcn component you expect is absent it was removed deliberately — re-add it from git history rather than assuming it was never there. Public-marketing chrome lives in `src/components/public/` (`PublicNav`, `PublicFooter`, `PublicLayout`).
- **Brand assets:** live logo `public/brand/swim-sight-logo.png` (256px, used in nav/footer); social preview `public/og-swim-sight-3d.png` (1200×630). **There is no hero image** — the homepage hero is typographic by design (the flat-modernist rebuild), and nothing in `src/` references a hero asset. High-res source reference (logo, concept screenshots, the Coach Studio mockup) lives in `docs/design-reference/` — reference only, never served.
- **Report templates / layout:** `src/components/reports/PrintableReport.jsx` (PDF/print, also what the swimmer portal renders) and `src/pages/SharedReportPage.jsx` (the public shared-link view, which renders from the payload sanitised by `src/lib/sanitizeAIReport.js`). Public share safety is enforced server-side in `api/shared-reports/[token].js` + guarded by `npm run test:public-report-safety`.
